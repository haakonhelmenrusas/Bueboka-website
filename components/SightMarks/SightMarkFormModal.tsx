...
export function SightMarkFormModal({ open, onClose, onSave, onDelete, bows, arrows, initialData }: SightMarkFormModalProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!selectedBowId) { setError('Du må velge en bue'); return; }
		if (!selectedArrowId) { setError('Du må velge et pilsett'); return; }
		if (distance <= 0 || mark <= 0) { setError('Vennligst fyll ut avstand og siktemerke med positive verdier'); return; }
			...
			{!isEditing && (
					<div className={styles.infoBox}>
						...
						<p className={styles.infoText}>
							Siktemerker hjelper deg å treffe blinken på ulike avstander. Du legger inn en avstand og tilhørende
							merke på siktet, og systemet beregner automatisk optimale merker for alle andre avstander basert
							på buens og pilens egenskaper. Jo flere merker du registrerer, desto nøyaktigere blir beregningene.
						</p>
					</div>
				)}
				...
				<form onSubmit={handleSubmit} className={styles.form}>
					<div className={styles.field}>
						<label className={styles.label} htmlFor="sm-name">Navn (valgfritt)</label>
						<input
							id="sm-name"
							type="text"
					...
					{isEditing && <div className={styles.divider}><span className={styles.dividerLabel}>Legg til nytt merke</span></div>}
					...
					<div className={styles.footer}>
						...
						<div className={styles.footerActions}>
							...
							<Button label={isEditing ? 'Legg til nytt merke' : 'Lagre merke'} type="submit" loading={loading} disabled={busy || noBows || noArrows} />
						</div>
					</div>
				</form>
		</Modal>
